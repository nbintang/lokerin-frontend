...
export const useApplyJob = (id: string) => {
  ...
  return useMutation({
    ...
    mutationFn: async () =>
      await lokerinAPI.post<ApplyJobResponse>(`/jobs/apply/${id}`),
    onMutate: () => {
    ...
    onSuccess: (res, _variables, context) => {
      ...
      queryClient.invalidateQueries({ queryKey: ["jobs", false] });
      queryClient.invalidateQueries({ queryKey: ["applied-jobs", id] });
      router.push(`/applier/dashboard/jobs`);
      setAppearApplySuccess(true, status);
    },
